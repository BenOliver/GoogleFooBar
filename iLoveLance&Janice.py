...
huge fans of the space soap opera "Lance & Janice". You know how much Commander Lambda hates waste, so if you can prove that these minions are wasting her time passing 
...
Write a function called answer(s) which takes in a string and returns the deciphered string so you can show the commander proof that these minions are talking about "Lance & 
...
Output:
    (string) "did you see last night's episode?"edit 
...
def myDecoder(sentence):
    newSent=''
    for let in sentence:
        if let in string.ascii_lowercase:
            newSent=newSent+string.ascii_lowercase[-string.ascii_lowercase.index(let)-1]
        else:
            newSent=newSent+let
    return newSent
