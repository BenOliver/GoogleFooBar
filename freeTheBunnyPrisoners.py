...
def findNextSmallest(num):
    '''find the next smallest number that
    also uses the same number of '1' bits'''
    numBits=bin(num).count('1')
    result=num-1 
    while(bin(result).count('1')!=(numBits)):
        
        result-=1
    return result

def findStart(numBunnies,reqBunnies):
    start=0
    for i in range(reqBunnies-1,numBunnies):
        start=start+(1<<i)
    return start

def findEnd(numBunnies,reqBunnies):
    end=0
    for i in range(numBunnies-reqBunnies+1):
        end=end+(1<<i)
    return end
    
def main(numBunnies,reqBunnies):
    N,R=numBunnies,reqBunnies
    binList=[]
    start=(findStart(N,R))
    end=findEnd(N,R)
    binList.append(start)
    print('The start is :{}\nThe end is :{}'.format(bin(start),bin(end)))
    
    while binList[-1]!=end:
#         print(bin(binList[-1])[2:].zfill(7))
        binList.append(findNextSmallest(binList[-1]))
...
#print(findNextSmallest(int('0b11000',2),4))
